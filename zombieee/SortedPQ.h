...
public:
    ...
    virtual void push(const TYPE &val)
    {
        ...
        while (!found && mid != data.end())
        {
            mid = beg + (end - beg) / 2;

            //  mid = val
            if (!this->compare(val, *mid) && !this->compare(*mid, val))
            {
                found = true;
            } // mid > val
            else if (this->compare(val, *mid))
            {
                if (mid == data.begin())
                {
                    found = true;
                ...
                {
                    if (this->compare(*(mid - 1), val) || (!this->compare(val, *(mid - 1)) && !this->compare(*(mid - 1), val)))
                    {
                        found = true;
                    }
                    else
