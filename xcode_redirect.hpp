...
inline char *get_filename_and_move_count(int &optind,
                                         char *argv[], int &move_count);
...
inline void xcode_redirect(int &argc, char *argv[], int optind = 1)
{
    ...
    while (optind < argc)
    {
        ...
        case '2': // redirect error formatted as '2> file' or '2>file'
            ...
            {
                errfile =
                    get_filename_and_move_count(optind, argv, move_count);
                if (!freopen(errfile, "w", stderr))
                {
                    std::cerr << "Unable to open error file: " << errfile
                              << std::endl;
                    exit(1);
                } // if
            }     // if
...
inline char *get_filename_and_move_count(
    int &optind, char *argv[], int &move_count)
{
    if (strlen(argv[optind]) > 2)
    {
        move_count = 1;
        // Must offset by 1 character (the <), offset by more if they put
        // space(s) before filename
        int offset = 1;
